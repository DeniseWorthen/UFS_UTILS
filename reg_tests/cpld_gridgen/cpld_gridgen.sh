...
if [ $RESNAME = 008 ]; then
    ...
    if [ $DO_POSTWGTS == .true. ]; then
        ...
        #atmos river mom6 domain; 1/12 deg rectilinear
        ncremap -g ar.0p08.SCRIP.nc -G latlon=1081,1920#snwe=$ll_lat,$ur_lat,$ll_lon,$ur_lon#lat_typ=uni
    #fi
fi
